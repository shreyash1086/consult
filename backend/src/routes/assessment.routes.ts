...
router.get('/:id', protect([Role.CANDIDATE]), async (req, res) => {
  ...
  const { modelAnswer, cachedOptimalTrajectory, ...safeScenario } = scenario as any;
...
router.get('/submissions/:id', protect([Role.CANDIDATE]), async (req, res) => {
  ...
  if (submission.status !== SubmissionStatus.EVALUATED) {
    const { scenario: { modelAnswer, cachedOptimalTrajectory, ...safeScenario }, ...safeSubmission } = submission as any;
    return res.json({ 
      ...safeSubmission, 
