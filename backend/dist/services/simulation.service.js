...
export const runSimulation = async (submissionId) => {
    try {
        ...
        let optimalTrajectory = scenario.cachedOptimalTrajectory;
        if (!optimalTrajectory) {
            optimalTrajectory = await generateOptimalStrategy(scenario, initialState);
            await prisma.scenario.update({
                where: { id: scenario.id },
                data: { cachedOptimalTrajectory: optimalTrajectory }
            });
        }
        ...
        await prisma.simulationRun.update({
            where: { submissionId },
            data: {
                candidateKPIs: candidateYear1,
                optimalKPIs: optimalYear1,
...
async function extractCandidateActions(consultationText, state) {
    const prompt = `
...
function simulateActions(state, actions) {
    let s = { ...state };
...
async function generateOptimalStrategy(scenario, initialState) {
    const prompt = `
...
async function generateDivergenceAnalysis(scenario, initialState, candidateActions, optimalActions) {
    const points = [];
...
function calculateOutcomeScore(initial, candidate, optimal) {
    const kpis = Object.keys(initial);
    let totalKpiScore = 0;
